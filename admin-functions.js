...
// دالة الحصول على رمز المصادقة (متوافقة مع script.js)
function getAuthToken() {
    // البحث في sessionStorage أولاً (كما في script.js)
    const bloomToken = sessionStorage.getItem('bloom_token');
    if (bloomToken) return bloomToken;
    
    // البحث في localStorage كبديل
    const localToken = localStorage.getItem('token');
    if (localToken) return localToken;
    
    // البحث في currentUser
    try {
...
async function loadAnnouncementForAdmin() {
    try {
        ...
        if (visible) visible.checked = !!(a.is_visible !== undefined ? a.is_visible : a.isVisible);
        if (title) title.value = a.title || '';
        if (content) content.value = a.content || '';
        if (discount) discount.value = (a.discount_percent !== undefined ? a.discount_percent : a.discountPercent) || 0;
        if (applyDiscount) applyDiscount.checked = !!(a.apply_discount !== undefined ? a.apply_discount : a.applyDiscount);
            ...
            prev.innerHTML = `
                <div class="announcement-card">
                  ...
                  <div class="announcement-body">
                      <h3>${a.title || ''}</h3>
                      <p>${a.content || ''}</p>
                      ${(a.apply_discount !== undefined ? a.apply_discount : a.applyDiscount) && ((a.discount_percent !== undefined ? a.discount_percent : a.discountPercent) || 0)>0 ? `<div class="announcement-discount">خصم ${a.discount_percent !== undefined ? a.discount_percent : a.discountPercent}% مفعل</div>` : ''}
                  </div>
                </div>`;
        }
    } catch (e) {
...
async function handleSaveAnnouncement(event){
    ...
    try{
        ...
        const r = await fetch('/api/admin/announcement', { method:'POST', body: fd, headers: { 'Authorization': `Bearer ${getAuthToken()}` } });
        const j = await r.json();
